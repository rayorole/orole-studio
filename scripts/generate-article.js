...
const casesFeeds = process.env.cases_FEEDS?.split(",");
...
---`;
	...
	fs.writeFileSync(`./src/content/cases/${fileName}.md`, newContent);
}

// https://cases.feedspot.com/rss_directory/
let feeds = await getAllFeeds(casesFeeds);
...
const articleTags = await generateText(`Create a comma separated list of tags (single words) for this cases article: "${item.body}"`, 10);
